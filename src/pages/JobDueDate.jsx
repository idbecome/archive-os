...
import { 
    ...
    User as UserIcon, Trash2, Edit3, ArrowLeft, CalendarDays, Repeat, UserPlus, LayoutGrid, X, Hash, ChevronLeft, Settings, Activity, Timer, Check, Info
} from 'lucide-react';
...
export default function JobDueDate({ currentUser, users, departments, hasPermission, isDarkMode, onCopy }) {
    ...
    const [issueForm, setIssueForm] = useState({ id: null, note: '', detail: '', createdAt: '', taskId: null, assignedTo: null, isAutoGenerated: false });
    ...
    const userBlocks = useMemo(() => {
        ...
        const visibleJobs = jobs.filter(job => {
            ...
            const hasIssueForMe = (job.issues || []).some(i => i.assignedTo === currentUser.username);
            
            return isOwner || isAssigned || isPublic || isDept || isSpecific || isAdmin || hasIssueForMe;
        });
            ...
            const allIssues = [
                ...visibleJobs.flatMap(task => {
                    ...
                    const taskIssues = (task.issues || [])
                        .filter(issue => issue.assignedTo === u.username || issue.owner === u.username || (task.assignedTo === u.username && issue.assignedTo === u.username))
                        .map(issue => ({ 
                            ...issue, 
                    ...
                    if (task.assignedTo === u.username && checkIsOverdue(task, selectedMonth) && task.status !== 'done' && !hasOverdueInIssues) {
                        taskIssues.push({
                ...
                ...independentIssues.filter(i => i.assignedTo === u.username || i.owner === u.username).map(i => ({
                    ...i,
    ...
    const handleOpenIssueModal = (taskId, issue = null) => {
        if (issue) {
            setIssueForm({ 
                ...
                assignedTo: issue.assignedTo || selectedUser || currentUser.username
            });
        } else {
            setIssueForm({ 
                id: null, 
                ...
                assignedTo: selectedUser || currentUser.username
            });
        }
    ...
    const handleSaveIssue = async () => {
        if (issueForm.taskId && issueForm.taskId !== 0 && issueForm.taskId !== '0') {
            ...
            let updatedIssues;
            if (issueForm.id) {
                updatedIssues = (task.issues || []).map(i => i.id === issueForm.id ? { ...i, note: issueForm.note, detail: issueForm.detail, createdAt: new Date(issueForm.createdAt).toISOString(), assignedTo: issueForm.assignedTo, owner: i.owner || currentUser.username } : i);
            } else {
                const newIssue = { id: Date.now(), note: issueForm.note, detail: issueForm.detail, createdAt: new Date(issueForm.createdAt).toISOString(), status: 'pending', progress: 0, history: [], taskId: issueForm.taskId, created_at: new Date().toISOString(), assignedTo: issueForm.assignedTo, owner: currentUser.username };
            ...
            if (issueForm.id) {
                ...
                const updatedIssue = { ...original, note: issueForm.note, detail: issueForm.detail, createdAt: new Date(issueForm.createdAt).toISOString(), assignedTo: issueForm.assignedTo, owner: original.owner || currentUser.username };
                ...
                const newIssue = { note: issueForm.note, detail: issueForm.detail, createdAt: new Date(issueForm.createdAt).toISOString(), status: 'pending', progress: 0, assignedTo: issueForm.assignedTo, owner: currentUser.username, history: [] };
                                                        ...
                                                        >
                                                                        ...
                                                                        return (
                                                                            <button 
                                                                                ...
                                                                                onClick={() => isDone 
                                                                                    ...
                                                                                    : handleInitiateStatusUpdate(issue.taskId, issue.id, step.id)
                                                                                }
                                                            ...
                                                            <td className="px-6 py-5 text-right rounded-r-[2rem]">
                                                                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    {!issue.isAutoGenerated && hasPermission('job-due-date', 'edit') && (
                                                                        <button 
                    ...
                    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl">
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                                {users.filter(u => !monitoredPICs.includes(u.username)).map(u => (
                                    <button 
                                        key={u.username}
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-2xl">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    >
                                        {users.map(u => (
                                            <option key={u.username} value={u.username}>{u.name} ({u.department})</option>
                                        ))}
                                    </select>
                                </div>
                                <div className="space-y-1">
...
function TaskItem({ task, selectedMonth, onToggle, onEdit, onDelete, onUpdateIssues, onUpdateKendala, hasPermission }) {
    const isOverdue = useMemo(() => {
        ...
        if (task.type === 'recurring') {
            ...
            const day = new Date(task.dueDate).getDate();
            taskDate = new Date(selectedMonth.getFullYear(), selectedMonth.getMonth(), day);
    ...
    return (
        <div className={`group relative p-4 rounded-3xl border transition-all ${
            ...
            <div className="flex items-start gap-3">
                <button 
                    onClick={() => hasPermission('job-due-date', 'edit') && onToggle(task.id)}
