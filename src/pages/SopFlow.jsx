        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredFlows.map(flow => (
                    <Card key={flow.id} onClick={() => setSelectedFlow(flow)} className="group hover:border-indigo-500 transition-all cursor-pointer relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={(e) => { e.stopPropagation(); setEditingFlow(flow); setForm(flow); setIsModalOpen(true); }} className="p-2 text-slate-400 hover:text-blue-600"><Edit3 size={16} /></button>
                                <button onClick={() => handleDelete(flow.id)} className="p-2 text-slate-400 hover:text-red-600"><Trash2 size={16} /></button>
                            </div>
                        </div>
            ...
            <Modal
                ...
                size="max-w-5xl"
            >
                <div className="pt-24 h-[70vh]">
                    {selectedFlow?.visual_config ? (
                        <WorkflowViewer
                            nodes={selectedFlow.visual_config.nodes}
                            edges={selectedFlow.visual_config.edges}
                        />
                    ) : (
                        <div className="flex flex-col items-center justify-center h-full text-slate-400">
                            <Info size={48} className="mb-4 opacity-20" />
                            <p className="text-sm font-bold uppercase tracking-widest">Visualisasi tidak tersedia untuk SOP ini</p>
                        </div>
                    )}
                </div>
            </Modal>
